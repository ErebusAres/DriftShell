...
const NARRATIVE_STEPS = [
  ...
  // NOTE: Keep narrative steps story-only; command metadata should not live in this list.
  {
    id: "mesh_explore",
...
const REGION_DEFS = [
  ...
  },
  {
    ...
    name: "Secure Core",
    nodes: ["core.relic", "rogue.core"],
    unlock: { requires: ["corporateNet"], flags: ["forked"], nodes: [] },
...
let LOCAL_BADNAME_WARNED = false;
...
function isSimpleDiskScriptName(name) {
  // Local disk files are transport only: simple base names ending with .s (no prefixes, dots, or namespaces).
  return /^[A-Za-z0-9_-]+\.s$/.test(String(name || ""));
}
...
function upsertUserScript(rawName, sec, code, zoneHint) {
  ...
  if (isCanonicalScriptName(rawName)) {
    state.userScripts[rawName] = { owner: state.handle, name: rawName, sec, code };
    return rawName;
  }
...
async function backfillLocalFolderFromDrive(options) {
  // No-op: exports are disabled to keep disk filenames simple and free of namespaces.
  return;
}
...
async function syncLocalFolder() {
  ...
  try {
    for await (const entry of localFolderHandle.values()) {
      ...
      if (!isSimpleDiskScriptName(name)) {
        if (!LOCAL_BADNAME_WARNED) {
          LOCAL_BADNAME_WARNED = true;
          chatPost({ channel: "#kernel", from: "sys", kind: "system", body: "local sync: ignoring legacy names with prefixes." });
        }
        continue;
      }
      ...
      if (!base) continue;
      const canonicalKey = canonicalScriptName(`${base}.s`, "local").canonical;
      const prior = state.userScripts[canonicalKey];
      const secMatch = text.match(/@sec\\s+(FULLSEC|HIGHSEC|MIDSEC|LOWSEC|NULLSEC)/i);
      const sec = secMatch ? secMatch[1].toUpperCase() : "FULLSEC";
      const savedKey = upsertUserScript(canonicalKey, sec, text, "local");
      seen.add(name);
      const entryState = state.userScripts[savedKey];
      if (!prior) added += 1;
      else if (!entryState || entryState.code !== text || entryState.sec !== sec) updated += 1;
      else unchanged += 1;
...
async function mirrorDownloadToLocalFolder(locName, fileName, entry) {
  // Disabled: no export/mirror writes to disk.
  return;
}
...
async function mirrorUserScriptToLocalFolder(scriptName, code) {
  ...
  if (!key) return;
  const base = scriptBaseFromKey(key) || "script";
  const fileName = `${base}.s`;
  const content = String(code || "");
  try {
    await writeLocalMirrorFile(fileName, content);
    localFileMeta.set(fileName, { lastModified: Date.now(), size: content.length, scriptKey: key });
  } catch (err) {
...
async function pollLocalFolderChanges() {
  ...
  for await (const entry of localFolderHandle.values()) {
    ...
    const canonicalKey = canonicalScriptName(`${scriptName}.s`, "local").canonical;
    const secMatch = text.match(/@sec\s+(FULLSEC|HIGHSEC|MIDSEC|LOWSEC|NULLSEC)/i);
    const sec = secMatch ? secMatch[1].toUpperCase() : "FULLSEC";
    const existing = state.userScripts[canonicalKey];
    const savedKey = upsertUserScript(canonicalKey, sec, text, "local");
    updateLocalFileMeta(name, file, savedKey);
    const changed = !existing || existing.code !== text || existing.sec !== sec;
    if (changed) {
      scriptUpdates += 1;
    }
  }
  ...
  if (npcId === "juniper") {
    ...
    if (state.flags.has("q_juniper_mask_done")) {
      chatPost({
        channel: dmChannel(npcId),
        from: "juniper",
        body: "You already cashed the only job I had. Stay in the exchange and keep scanning; pull tools when you see them.",
      });
    }
    return;
  }
...
const LOCS = {
  ...
  "lattice.cache": {
    title: "LATTICE.CACHE",
    desc: [
      "A cache of old relays stacked under the archive.",
      "Signals smear here; patience and trust keep them readable.",
    ],
    requirements: { flags: ["trace_open", "lattice_sigil"], trust: 2 },
    locks: [
...
const TUTORIAL_STEPS = [
  ...
  /* Tutorial steps beyond early Public Net competency removed (kept commented for reference).
  {
    id: "t_mask",
    title: "Mint A Mask",
  ...
  {
    id: "t_fork",
    title: "Fork The Relay",
    hint: "Run `download fork.s`, then `call kit.fork`.",
    check: () => state.flags.has("forked"),
    onStart: () =>
      chatPost({
        channel: "#kernel",
        from: "switchboard",
        body: "Split the relay channel. Forking exposes the core route when you're ready.",
      }),
  },
  {
    id: "t_cache",
...
function tutorialAdvance() {
  ...
  if (state.tutorial.completed.has("t_exchange")) {
...
function printTrustStatus(options) {
  ...
  NARRATIVE_STEPS.filter((step) => step && typeof step === "object" && step.id && Array.isArray(step.nodes)).forEach((step) => {
...
function storyChatTick() {
  ...
  if (state.unlocked.has("training.node") && !state.flags.has("tutorial_training_done")) {
  ...
  // Progression cohesion: once the operator has the full relic kit (fork + keys),
  // advance into the core chapter so the Secure Core region can become visible naturally.
  if (state.storyState && state.storyState.current === "glitch_arc") {
    const relicReady =
      state.flags.has("forked") &&
      state.inventory.has("relay.shard") &&
      state.inventory.has("relic.key");
    if (relicReady) {
      storyEnsureAtLeast("rogue_finale", "ready:relic");
      RegionManager.bootstrap({ silent: true });
    }
  }
